...
import styles from "./Securitate.module.scss";
...
const Security = () => {
  ...
  const isFormValid = (field) => {
    ...
    if (field === "oldPassword") {
      if (oldPassword.length < 7) {
        return "Parola trebuie sa aiba minim 7 caractere";
      }
    }
    ...
    if (field === "all") {
      if (
        ...
        return "Eroare la schimbarea parolelor!";
      }
    }
  ...
  const handleResetPassword = async (e, field) => {
    ...
    if (!isFormValid(field)) {
      ...
      try {
        ...
        setAlert({
          type: "warning",
          message: "Parola veche este gresita",
        });
      }
    }
  };
  ...
  const handleDeactivate = async (e) => {
    ...
    e.stopPropagation();
    try {
      console.log("deactivate accounts");
